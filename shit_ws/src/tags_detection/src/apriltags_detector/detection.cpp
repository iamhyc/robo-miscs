...
/* not for first stage information about the point
...
*/  detectPos(detection);
  ...
  void detectPos(AprilTags::TagDetection& detection) const  {
    ...
    if (id < 4 && id >= 0){
      ...
      if(detection.cxy.first >= 100 && detection.cxy.first <= 200 && detection.cxy.second >= 150
               && detection.cxy.second <= 200){
        tags_array[id] = 1;
      }
      if(detection.cxy.first >= 100 && detection.cxy.first <= 200 && detection.cxy.second >= 400
               && detection.cxy.second <= 450){
        tags_array[id] = 2;
      }
      if(detection.cxy.first >= 500 && detection.cxy.first <= 600 && detection.cxy.second >= 150
               && detection.cxy.second <= 200){
        tags_array[id] = 3;
      }
      if(detection.cxy.first >= 500 && detection.cxy.first <= 600 && detection.cxy.second >= 400
               && detection.cxy.second <= 450){
        tags_array[id] = 4;
      }
    }
  ...
  void processImage(cv::Mat& image, cv::Mat& image_gray) {
    ...
    if (m_draw) {
      ...
      cv::rectangle(image, cv::Point2f(100.0, 150.0), cv::Point2f(200.0, 200.0), cv::Scalar(0,0,255,0) );
      cv::rectangle(image, cv::Point2f(100.0, 400.0), cv::Point2f(200.0, 450.0), cv::Scalar(0,0,255,0) );
      cv::rectangle(image, cv::Point2f(500.0, 150.0), cv::Point2f(600.0, 200.0), cv::Scalar(0,0,255,0) );
      cv::rectangle(image, cv::Point2f(500.0, 400.0), cv::Point2f(600.0, 450.0), cv::Scalar(0,0,255,0) );
  ...
  void loop() {
    ...
    int frame = 0;
    double last_t = tic();
    ros::init(argc, argv, "tags_publisher");    
    ros::Publisher pub = n.advertise<std_msgs::String>("tag_message", 1000);
...
//      loop_rate.sleep();
      // cv::waitKey(1);
        tags_array[0], tags_array[1], tags_array[2], tags_array[3]  = 0;
