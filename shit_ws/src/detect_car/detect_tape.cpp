...
/*		GaussianBlur( combine, combine2, Size( 3, 3 ), 1, 1 );	
	...
	{	
		...
		if(boundRect[i].size.width/boundRect[i].size.height > 0.6 
			...
			//square, area limit(13000, 5000), (0.6,1.3), matching. (0.8, 0.95) 
		...
		M = getRotationMatrix2D(boundRect[i].center, angle, 1.0);
        // perform the affine transformation
		warpAffine(threshold2, rotated, M, threshold2.size(), INTER_CUBIC);
        // crop the resulting image
		getRectSubPix(rotated, rect_size, boundRect[i].center, roi);
		double s = sum(roi)[0];
