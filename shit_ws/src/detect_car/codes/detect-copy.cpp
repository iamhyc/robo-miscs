...
VideoCapture cap = VideoCapture("/home/leory/shit_ws/src/detect_car/imgs/pink_car.avi"); // VC(0)
	//VideoCapture cap(0); 	//while(1){
	if(!cap.isOpened()){
		cout << "error!" << endl;
		return 0;
	// Mat grayframe;
...
void detect(VideoCapture m_cap, Data& data_circle, Data& data_car, Data& data_tape){
	 //VideoCapture cap(0); // VC(0)
	Mat frame;
	m_cap.read(frame);
	//Mat frame = imread("/home/leory/shit_ws/src/detect_car/imgs/car_pink.jpg", CV_LOAD_IMAGE_COLOR);
	if(! frame.data )                            
	{
	...
	//find_car(frame, data_car, low_hsv_car, high_hsv_car);
	//find_tape(frame,data_tape,low_hsv_tape, high_hsv_tape);
	imshow("frame", frame);
	waitKey(0);
}
void solveDistance (Mat frame, vector<Point2f> image_points, Data& data){
...
void find(Mat frame, Data& data, int low_hsv[3], int high_hsv[3]){
	...
	{		
		...
		//cout << area0/area1 << ", "<< area0 << endl;
		if(area0 / area1>0.90 && area0 > 1000){//0.85
	...
	{	
		...
		vector<Point2f> image_points;  //point2d
...
void solveDistance_rotated (Mat frame, RotatedRect rect, Data& data){
  ...
  std::vector<cv::Point2d> image_points;
    model_points.push_back(cv::Point3d(0.0f, 0.0f, 0.4f));            
    model_points.push_back(cv::Point3d(0.6f *cos(rect.angle), 0.6f * sin(rect.angle), 0.4f));        
    model_points.push_back(cv::Point3d(0.4f *cos(180-rect.angle), 0.4f * sin(180-rect.angle), 0.4f));        
    model_points.push_back(cv::Point3d(0.6f * sin(rect.angle), -0.6f *cos(rect.angle), 0.4f));  
...
void find_car(Mat frame, Data& data, int low_hsv[3], int high_hsv[3]){
	Mat HSV, M, rotated, roi, roi_hsv, roi_threshold;
	cvtColor(frame, HSV, CV_BGR2HSV);
	imshow("HSV",HSV);
	Mat element = getStructuringElement(MORPH_RECT, Size(3, 3));//(5,5)
	Mat frame_threshold;
	Mat frame_threshold2; 
	vector<vector<Point> > contours;
	...
	//dilate(frame_threshold,frame_threshold2,element,Point(-1,-1), 2);//dilate twice	
  ...
  { 
    ...
    if(boundRect[i].size.width/boundRect[i].size.height > 0.5 
      ...
      area1 >= 8000 &&area0 / area1>0.6 && area0 / area1 < 0.95) {
      	cout << area1 << endl;
      //square, area limit(13000, 5000), (0.6,1.3), matching. (0.74:0.8, 0.95) 
      float angle = boundRect[i].angle;
      Size rect_size = boundRect[i].size;
...
// 	dilate(roi_threshold,roi_threshold,element,Point(-1,-1), 4);//dilate twice
// 	imshow("threshold", roi_threshold);
// 	roi_threshold = roi_threshold; 
// 	findContours( roi_threshold, contours_roi, hierarchy, CV_RETR_EXTERNAL , CV_CHAIN_APPROX_SIMPLE, Point(0, 0) );
// 	vector<std::vector<Point> > roi_contours_new;
// 	Rect tmp;
// 	vector<Rect> new_Rect;
...
// 		Point2f center;
// 		float radius = 0;
// 		minEnclosingCircle(Mat(contours_roi[i]), center, radius);
// 		double area0 = contourArea(contours_roi[i]);
// 		double area1 = 3.1415926 *  radius * radius;
// 		tmp = boundingRect( Mat(contours_roi[i]) );
// 		// /cout << area0/area1 << ", "<< area0 << endl;
// 		if(area0 / area1>0.40 && area0 > 5000){//0.85
// 			roi_contours_new.push_back(contours_roi[i]);
// 			new_Rect.push_back(tmp);
			
// 		}
// 	}
// 	for(int i = 0; i< roi_contours_new.size(); i++ )
// 	{	
// 		Rect rect = new_Rect[i];
// 		drawContours( frame, roi_contours_new, i, Scalar(0,255,255), 2, 8, hierarchy, 0, Point() );
// 		//drawContours( frame, boundRect, i, Scalar(0,255,255), 2, 8, hierarchy, 0, Point() );
// 		Point2f bl = ((Point2f)rect.br() - Point2f(rect.width,0));
// 		Point2f tr = ((Point2f)rect.tl() + Point2f(rect.width,0));
...
//     double area0 = contourArea(contours[i]);
//     double area1 = boundRect[i].size.width * boundRect[i].size.height;
//     //cout << area0 / area1 << ", " << area1 << endl;
//     if( area1 >= 5000 &&area0 / area1>0.4 && area0 / area1 < 0.95){
//       //square, area limit(13000, 5000), (0.6,1.3), matching. (0.74:0.8, 0.95) 
//       float angle = boundRect[i].angle;
//       Size rect_size = boundRect[i].size;
//       contours_new.push_back(contours[i]);
