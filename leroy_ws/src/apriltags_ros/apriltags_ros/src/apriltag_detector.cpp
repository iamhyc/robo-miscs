...
void AprilTagDetector::imageCb(const sensor_msgs::ImageConstPtr& msg, const sensor_msgs::CameraInfoConstPtr& cam_info){
 ...
 // ROS_DEBUG("%d tag detected", (int)detections.size());
  ...
  if (projected_optics_) {
    // use projected focal length and principdescription_itral point
  ...
  if (!detections.empty()){
      ...
      if(detection.id >= 4 || detection.id < 0){
        continue;
      }
      cv::circle(image, cv::Point2f(320, 240), 8, cv::Scalar(0,0,255,0), 2);
...
/*      double tag_size = description.size();
    ...
    tf_pub_.sendTransform(tf::StampedTransform(tag_transform, tag_transform.stamp_, tag_transform.frame_id_, description.frame_name()));
     ...
     strSt << "position(" <<tag_pose.pose.position.x <<", " <<tag_pose.pose.position.y<<", "<< tag_pose.pose.position.z <" )";
 ...
 // cv::Size frameSize(640, 480);
  /*cv::VideoWriter oVideoWriter ("MyVideo.avi", CV_FOURCC('P','I','M','1'), 10, frameSize, true);
