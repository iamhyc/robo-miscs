...
void TagDetection::draw(cv::Mat& image) const {
  ...
  // plot outline
/*  cv::line(image, cv::Point2f(p1.first, p1.second), cv::Point2f(p2.first, p2.second), cv::Scalar(0,0,0,0) );
  cv::line(image, cv::Poin#include <iostream>t2f(p2.first, p2.second), cv::Point2f(p3.first, p3.second), cv::Scalar(0,255,0,0) );
