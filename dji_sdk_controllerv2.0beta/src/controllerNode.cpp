...
{
	enum LogicState this_state = state;
	enum LogicState next_state = state;
	...
	while(true)//next for some status judgement
	{
		...
		case INIT:
			...
			next_state = GRAB_MODE;
			break;
		case GRAB_MODE:
			call_proxy("grab.auto");
				call_proxy("takeoff", true);
			next_state = PATROL_MODE;
			break;
		case DROP_MODE:
